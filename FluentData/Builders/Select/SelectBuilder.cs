...
namespace FluentData
{
	internal class SelectBuilder<TEntity> : ISelectBuilder<TEntity>
	{
		...
		private IDbCommand GetPreparedDbCommand()
		{
            ...
            //设置查询参数
            if (Data.NamedParameters.Count > 0)
            {
                foreach (var namedParameter in Data.NamedParameters)
                {
                    Data.Command.Parameter(namedParameter.name, namedParameter.value,
                        namedParameter.parameterType, namedParameter.direction, namedParameter.size);
                }                    
            }
            if (Data.ObjectParameters.Count > 0)
            {
                Data.Command.Parameters(Data.ObjectParameters.ToArray());
            }
		...
		{
            Data.NamedParameters.Add(new DbCommandParameter { name = name, value = value, parameterType = parameterType, direction = direction, size = size });			
			return this;
		}
		...
		public ISelectBuilder<TEntity> Parameters(params object[] parameters)
		{
            if (parameters != null)
            {
                ...
                {
                    Data.ObjectParameters.Add(parameter);
                }
            }			
			return this;
		}
